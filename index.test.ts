...
describe('Replicate client', () => {
  ...
  describe('constructor', () => {
    ...
    test('Sets custom baseUrl', () => {
      const clientWithCustomBaseUrl = new Replicate({ baseUrl: 'https://example.com/', auth: 'test-token' });
      expect(clientWithCustomBaseUrl.baseUrl).toBe('https://example.com/');
    });

    test('Sets custom userAgent', () => {
      const clientWithCustomUserAgent = new Replicate({ userAgent: 'my-app/1.2.3', auth: 'test-token' });
  ...
  describe('models.get', () => {
    test('Calls the correct API route', async () => {
      ...
      expect(client[ 'instance' ]).toHaveBeenCalledWith('/models/replicate/hello-world', {
        method: 'GET',
      });
    });
  ...
  describe('predictions.get', () => {
    test('Calls the correct API route with the correct payload', async () => {
      ...
      expect(client[ 'instance' ]).toHaveBeenCalledWith('/predictions/rrr4z55ocneqzikepnug6xezpe', {
        method: 'GET',
      });
    });
  ...
  describe('predictions.list', () => {
    ...
    test('Paginates results', async () => {
      ...
      expect(client[ 'instance' ]).toHaveBeenCalledWith('https://api.replicate.com/v1/predictions?cursor=cD0yMDIyLTAxLTIxKzIzJTNBMTglM0EyNC41MzAzNTclMkIwMCUzQTAw', {
        method: 'GET',
      });
    });
  ...
  describe('run', () => {
    test('Calls the correct API routes', async () => {
      ...
      expect(client[ 'instance' ]).toHaveBeenCalledWith('/predictions/ufawqhfynnddngldkgtslldrkq', {
        method: 'GET',
      });
