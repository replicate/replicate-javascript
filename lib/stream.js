...
class Stream extends Readable {
  ...
  constructor(url, options) {
    if (!Readable) {
      throw new Error('Readable streams are not supported. Please use Node.js 18 or later, or install the readable-stream package.');
    }
  ...
  decode(line) {
    if (!line) {
      ...
      const sse = new ServerSentEvent(this.event, this.data.join('\n'), this.lastEventId);
